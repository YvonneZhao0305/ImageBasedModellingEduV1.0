...
/*第一个相机的内外参数*/  //归一化的坐标不考虑图像中心，不考虑畸变
...
bool  is_correct_pose (math::Matrix3d const &R1, math::Vec3d const & t1
    ...
    bool flag=false;
      ...
      *
      */
     math::Vec3d P_c1 = R1 * p3d + t1;
     math::Vec3d P_c2 = R2 * p3d + t2;
                   ...
                   , math::Vec3d & t)
{
     ...
     */
    E = K2.transpose() * F * K1;
//    math::Matrix< double, 3, 3> A, B, C;
//    math::matrix_svd(E, &A, &B,&C);
//    B(0,0) = (B(0,0) + B(1,1))/2;
//    B(1,1) = B(0,0);
//    B(2,2) = 0;
//    E = A * B * C.transpose();
     ...
     * 可以通过对本质矩阵进行分解来求得, E具有尺度等价性，可以采用更简单的方法　E=U*S*V',其中S是进行尺度归一化之后是diag(1,1,0)
     */

    math::Matrix<double, 3, 3> W(0.0);
    W(0, 1) = -1.0; W(1, 0) = 1.0; W(2, 2) = 1.0;//绕z轴旋转90°的旋转矩阵
    ...
    // 保证旋转矩阵 det(R) = 1 (instead of -1).旋转矩阵性质：正交且行列式为１
    ...
    std::cout<<"pratical R0: "<< poses[0].first <<"\n"
            <<"t0: " << poses[0].second<<"\n"
            <<"pratical R1: "<< poses[1].first <<"\n"
            <<"t1: " << poses[1].second<<"\n"
            <<"pratical R2: "<< poses[2].first <<"\n"
            <<"t2: " << poses[2].second<<"\n"
            <<"pratical R3: "<< poses[3].first <<"\n"
            <<"t3: " << poses[3].second<<"\n"
    << std::endl;
