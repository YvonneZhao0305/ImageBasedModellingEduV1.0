//Created by sway on 2018/8/29.
    ...
    *                       p2'Fp1=0,
    *
    * 其中p1, p2 为来自两个视角的匹配对的归一化坐标，并表示成齐次坐标形式，        p1 p2应该是像素坐标？
    * 即p1=[x1, y1, z1]', p2=[x2, y2, z2],将p1, p2的表达形式带入到
    * 上式中，可以得到如下表达形式
    *
    *          [x2] [f11, f12, f13] [x2, y2, z2]
    *          [y2] [f21, f22, f23]                = 0
    *          [z2] [f31, f32, f33]
    *
    * 进一步可以得到
...
/*    std::cout<<A.rows<<std::endl;*/
    ...
    math::matrix_svd<double, 8, 9>(A, nullptr, nullptr, &vv);//SVD得到的U、S、V,V的最后一列是A^T*A最小特征值对应的特征向量
