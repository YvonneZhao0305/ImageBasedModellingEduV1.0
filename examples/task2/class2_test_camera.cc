...
public:
    ...
    math::Vec2d projection(math::Vec3d const & p3d){
        ...
        /*todo*/
        // 世界坐标系到相机坐标系
        double xc = p3d[0] * R_[0] + p3d[1] * R_[1] + p3d[2] * R_[2] + t_[0];
        double yc = p3d[0] * R_[3] + p3d[1] * R_[4] + p3d[2] * R_[5] + t_[1];
        double zc = p3d[0] * R_[6] + p3d[1] * R_[7] + p3d[2] * R_[8] + t_[2];
        ...
        // 图像坐标系到屏幕坐标系
        math::Vec2d p;
        p[0] = f_ * x * distort_ratio + c_[0];
        p[1] = f_ * y * distort_ratio + c_[1];
    ...
    math::Vec3d dir_in_world(){
         ...
         * todo
         */
        dir[0] = R_[6], dir[1] = R_[7], dir[2] = R_[8];
        return dir;
    }
public:
